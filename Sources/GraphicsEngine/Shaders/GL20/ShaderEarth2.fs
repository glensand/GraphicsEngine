...
void main()
{
	...
	vec2 uv2Scale = vec2(1, 1);
	
	vec3 normalLoc = texture2D( texture2, uv1.xy * uv2Scale ).rgb;
    normalLoc = normalLoc * 2.0 - 1.0;	
	...
	vec3 normalGlob = normalLoc.x*t + normalLoc.y*b + normalLoc.z*vertexNormal;
	normalGlob = normalize(normalGlob);
	...
	for (int i = 0; i < 3; ++i)
	{
		...
		col += attenuation * color1.rgb * (calcDiffuse(lightCol, lightDir, normalGlob));
	}
